...
public class spline1 extends PApplet {
...
public void drawTrack() {
  ...
  if (points.size() >= 4) {
    for (int i = 0; i < points.size()-3; i++) {     
      for (int j = 0; j < 50; j++) {
        PVector prev = getPos(i+j/50.1f);
        PVector cur = getPos(i+(j+1)/50.1f);
        tr.stroke(255);
...
public void generateDists() {
  ...
  for (int i = 1; i<d.length; i++) {
    PVector prev = getPos((i-1)/(N+0.01f));
    PVector cur = getPos(i/(N+0.01f));
    d[i] = d[i-1] + dist(prev.x, prev.y, cur.x, cur.y);
  }
}
 
public PVector getPos(float u) {
  int i = PApplet.parseInt(u);
...
public float blendFunction(float p0, float p1, float p2, float p3, float t) {
...
public void keyPressed() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "spline1" };
    if (passedArgs != null) {
